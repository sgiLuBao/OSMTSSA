	...
	while (eval <= evaluations) {
		...
		for (i = 1; i <= popsize; i++) {
			for (j = 1; j <= (vertice - 1) * 3; j++) {
				pop2[i].edge[j] = pop[i].edge[j];
			}
			pop2[i].fit_value = pop[i].fit_value;
		}
		for (i = 1; i <= popsize / 2; i++) {
			for (j = r1; j <= r2; j++) {
				temp = pop2[i * 2 - 1].edge[j * 3];
				pop2[i * 2 - 1].edge[j * 3] = pop2[i * 2].edge[j * 3];
				pop2[i * 2].edge[j * 3] = temp;
			}
		}
		//mutation
		for (i = 1; i <= popsize; i++) {
			...
			if (r3 < pm) {
				r4 = rand() % (vertice - 1);
				if (r4 == 0) r4 = vertice - 1;
				r5 = rand() % 4;
				while (r5 == pop2[i].edge[r4 * 3]) r5 = rand() % 4;
				pop2[i].edge[r4 * 3] = r5;
			}
		}
		//selection
		for (i = 1; i <= popsize; i++) {
			for (j = 1; j <= (vertice - 1) * 3; j++) {
				min_tree[j] = pop2[i].edge[j];
			}
			pop2[i].fit_value = Getfitness();
			if (pop2[i].fit_value < min_fitness) {
				flag = 1;
				index = i;
				min_fitness = pop2[i].fit_value;
			}
			if (pop2[i].fit_value < pop[i].fit_value) {
				for (j = 1; j <= (vertice - 1) * 3; j++) {
					pop[i].edge[j] = pop2[i].edge[j];
				}
				pop[i].fit_value = pop2[i].fit_value;
			}
		}
