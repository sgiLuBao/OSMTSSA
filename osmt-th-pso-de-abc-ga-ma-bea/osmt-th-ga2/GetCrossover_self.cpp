int parent2[MAX];
int parent22[MAX];
void initial2(int n) //每个节点的组别初始化为自身
{
	int i;
	for (i = 1; i <= n; i++) {
		parent2[i]=i;
...
int find_set22(int x) //查找组别
{
	...
	{
		temp = parent22[x];
		parent22[x] = find_set22(temp); //压缩路径
		return parent22[x];
	}
}
...
struct tree1 
{
	int s,e;
	int line_style;
}par1[1010], par1_best[1010], sur1[2010], new_res1[1010], sur2[2010]; //存放当前粒子和自身历史最优粒子，便也排序,sur1存放非共同边,new_res1存放交叉后生成的新粒子
bool cmpp1(struct tree1 a,struct tree1 b) //按照粒子起点由小到大，终点由小到大的顺序排列粒子
{
...
//	printf("\n");
	...
	{ 
	  ...
	  {
         par1[i].s=pop[cur].edge[i*3-2]; //将当前粒子存入结构体中
		 par1[i].e=pop[cur].edge[i*3-1];
		 par1[i].line_style=pop[cur].edge[i*3];
		 if(par1[i].s>par1[i].e) //如果起点编号大于终点编号，则交换位置
		 {
			 mid=par1[i].s;
			 par1[i].s=par1[i].e;
			 par1[i].e=mid;
			 if(par1[i].line_style==0||par1[i].line_style==1) //连接方式互换
...
void GetCrossover_GA(int cur1, int cur2)
{
	double c;
	...
	if (c<cm)
	{
		for (i = 1; i <= (vertice - 1); i++)
		{
			par1[i].s = pop[cur1].edge[i * 3 - 2]; //将当前粒子存入结构体中
