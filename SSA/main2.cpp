...
//            c1_now=c1_now-c1_dec;
		...
		{	
			...
			int jrr=rand() % Np + 1;  //生成一个1-popsize的随机数
//			前期注重探索，因为ST_now由小变大 
			if(R>=ST_now){ 
			  	if(CRr<CRR || jrr==i){
			  		GetMutation_SSA(i);
				...
				double ALfa=rand()/(RAND_MAX+1.0);
				double CrRate=exp((0-i)/(ALfa*evaluations));
			  	GetCrossover_self_SSA(i,CrRate); //未变异的pop(i)与 pbest进行交叉 
			} 
		}
		sort(pop+1,pop+popsize+1,cmp);
		Dividepop();
